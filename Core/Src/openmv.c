...
uint8_t   CxErr=0,CyErr=0; //目标物体中心坐标的误差
/*数据接收函数*/
uint8_t Cx=0,Cy=0,Cw=0,Ch=0;
void Openmv_Receive_Place_Data(int16_t Com_Data){
  /*循环体变量*/
  uint8_t i;
	/*计数变量*/
  ...
  if(RxState==0&&Com_Data==0x2C)  //0x2c帧头
		{
			RxState=1;	//接收到帧头，进入下一个状态
			RxBuffer1[RxCounter1++]=Com_Data;
      HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin, GPIO_PIN_SET);
		}

	else if(RxState==1&&Com_Data==0x12)  //0x12帧头,进入下一个状态，开始接收数据
		{
      HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin, GPIO_PIN_RESET);
	...
	else if(RxState==2)			//接收数据状态
		{
			...
			if(RxCounter1>=10||Com_Data == 0x5B)       //RxBuffer1接受满了,接收数据结束
				{
					RxState=3;
					CxErr=RxBuffer1[RxCounter1-3];
					CyErr=RxBuffer1[RxCounter1-2];
					printf("%d\r   ",CxErr);
					printf("%d\r   ",CyErr);
					}
			}
